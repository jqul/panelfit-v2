                        ...
                        {deletingId === client.id ? (
                          ...
                          <>
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              className="px-2 hover:bg-ok/10 hover:border-ok/30 transition-colors"
                              title="Enviar por WhatsApp"
                              onClick={(e) => {
                                e.stopPropagation();
                                const url = `${window.location.origin}?c=${client.token}`;
                                const msg = encodeURIComponent(`Hola ${client.name}! 👋 Aquí tienes tu enlace personal de PanelFit con tu plan de entrenamiento:\n\n${url}`);
                                window.open(`https://wa.me/?text=${msg}`, '_blank');
                              }}
                            >
                              <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current text-ok" xmlns="http://www.w3.org/2000/svg">
